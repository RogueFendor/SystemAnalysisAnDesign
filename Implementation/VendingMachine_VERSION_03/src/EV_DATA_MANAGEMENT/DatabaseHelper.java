  ...
  */
    ...
    */
        ...
        ArrayList<String> config = new ArrayList<String>();
         try{
           ...
           while ( rs.next() ) {
             ...
             firstSearch = ""+id+","+name+","+password+","+email+","+phone;
    ...
    public ArrayList<String> getMachineConfig() {
        ...
        ArrayList<String> config = new ArrayList<String>();
         try{
             stmt = c.createStatement();
              rs = stmt.executeQuery( "SELECT * FROM SLOT_CONFIG;" );
             String firstSearch="";
             String secondSearch="";
             int ctr=0;
           while ( rs.next() ) {
    ...
    */
           ...
           System.out.println("Error in getPassword");
            ...
            try{
               ...
               rs.close();
                ...
                System.out.println("Closing finally:  "+e);
            
            }
        }
        System.out.println("returned ID FROM "+tableName+"="+maxID);
        // TODO implement here
      return maxID;
    }
    public String getPassword(String userName,String email){
      ...
      String password="";
      System.out.println("SELECT PASSWORD FROM TECHNICIAN WHERE USERNAME="+"'"+userName+"';");
      try{
        stmt = c.createStatement();
         rs= stmt.executeQuery("SELECT PASSWORD FROM TECHNICIAN WHERE USERNAME="+"'"+userName+"';");
         while ( rs.next() ) {
    ...
    */
    ...
    public double getFinanceStatus(){

      double status=0;
      try{
        stmt = c.createStatement();
         rs= stmt.executeQuery("SELECT MONEY FROM ACCOUNT;");
         while ( rs.next() ) {
         status = rs.getDouble("money");
        }
      } 
      catch ( SQLException e ) {
          this.setStatus(true);
