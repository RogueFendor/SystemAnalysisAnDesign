...
public class EVOserver implements Observer{
   ...
   public EVOserver(ObservableVals ov, EsyVendingGUI gui,VendingMachine vm )
   {
    ...
    public void setUpSystemControl(SystemAdvocate sa){
    ...
    public void update(Observable o, Object arg) {
      ...
      { /*
         ...
         if (ov.isStartSampling()==true){
            sa.setUpSystemControl();
         }
         if(ov.isGetSlotConfig()==true){
            if(vm.getSlot(gui.simulator.index).isIsUsed()==false){
             ...
             gui.simulator.setTextFields("Empty", "Empty");
                ...
                gui.simulator.setTextFields(n1[2],n2[2]);
                double temp = Double.parseDouble(n2[2] +"0");
                this.sa.Dispense(gui.simulator.getCurrentPrice());
                gui.simulator.setCurrentPrice(temp);
            }
         }
      }    
    }
